...
client.download_review_card_pdf(result.review_id, "report.pdf")
```
...
| `download_review_card()` | 一页纸的可交互报告，单文件 HTML，双击即开、断网可看（任何源格式） |
| `download_review_card_pdf()` | 同一页纸的 PDF 版，供存档签字 |

### 过程审计：把 Agent 的执行轨迹一起交上去

上面查的是**报告里的数字**。再交一份执行轨迹，还能查**过程**——哪一步没经过
验证器、哪一步声称已验证却有数据回放失败、哪些产出说不清来源。

```python
result = client.review("report.pdf", workflow_audit={"nodes": [
    {"label": "取2025年报营收",
     "type": "mx_ashare_finance_data",       # 调用的接口名
     "status": "UNVERIFIED",
     "inputs": [],                            # 上游步骤
     "output": {
         "records": [{"value": 1688.4, "unit": "亿元", "date": "2025-12-31",
                      "raw_ref": "call_abc",  # 这条数回得去的那次调用
                      "replay_ok": True}],
         "records_total": 1, "truncated": 0},
     "evidence_refs": ["sha256:…"],
     "verification": {"status": "", "loss": None, "errors": []},
     "error": ""},
    {"label": "算同比增速", "type": "compute", "status": "VERIFIED",
     "inputs": [{"label": "取2025年报营收"}],
     "output": {"records": [...], "records_total": 1, "truncated": 0},
     "verification": {"status": "passed", "loss": 0.0, "errors": []}},
]})
```

**不给不等于通过**：报告里那一节会明写「本次未提供 Agent 执行轨迹，这一块没有
审计」。轨迹几十 KB 塞不进 query，所以给了它会自动改走 JSON 信封（文档 base64）；
不给时请求体一个字节不变。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `label` | str | **唯一必填**。步骤名，报告里「问题步骤」那格 |
| `type` | str | 调用的接口名。有数据却不给它，会被判成「来源说不清」 |
| `status` | str | `VERIFIED`/`OK`/`DONE`；`UNVERIFIED`/`PENDING`/`SKIPPED`；`BLOCKED`/`FAILED`。默认 `PENDING`（"没跑到"） |
| `inputs` | list | `[{"label": "上游步骤名"}]`，断链时靠它往回追 |
| `output.records` | list | 逐条明细：`value` / `unit` / `date` / `raw_ref` / `replay_ok` |
...
| 你交的 | 判定 |
|---|---|
| 有 `records`，却既无 `type` 又无 `inputs` | `orphan` —— 数据凭空出现。`kind="object"` 豁免 |
| `records_total` ≠ `len(records)` | `contradiction` —— 申报数和明细是两套东西 |
| `status` 是 `VERIFIED`，却有记录 `replay_ok=false` | `contradiction` —— 两个事实互斥 |
| `status="UNVERIFIED"` | `unverified` —— 不是错误，是如实标注 |

**`replay_ok` 请如实填。** 没做过回放比对就别填 `true`——报一个自己没做过的
检查，比不报更糟。

> 这个参数只有 SDK 收，MCP 的 `sagax.review` **刻意不暴露**它：MCP 工具的入参
> 来自模型，模型自己交的轨迹就是自述，而这套东西的前提正是不拿自述当证据。
> 过程审计请在你的后端代码里调。
