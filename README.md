...
`evidence` 是判定的依据。**没有证据，文档里所有数字都算无源** —— 这不是缺陷，
是 P0 规则的直接结果：没有来源的数字过不了 P0，而 P0 不可关闭。
...
只有一个工具：**`sagax.review`** —— 给它一份文档路径和证据，它回高亮、comment、
...
sagax-amadeus review report.pdf --evidence ev.json --annotated out.pdf
...
```bash
sagax-amadeus review report.pdf --evidence ev.json || echo "不能发"
```
