...
把文件路径给它，并把稿子里数字的来源放进 `evidence`。

不给证据的话，文档里每个数字都算无源，评级必然是 D。这不是工具的毛病，是
"说不清出处的数字不该发出去"这条规则的直接结果，关不掉。
...
**Q：为什么每次都是 D？**
八成没给 `evidence`。没有证据，文档里每个数字都无源，P0 逐个开火。补证据，
不是关规则——P0 关不掉。
