...
- **证据是判定的依据。** 不给证据，文档里每个数字都是无源的，P0 直接开火，评级落到
  D。这不是误报，是如实反映——补证据，不是关规则，P0 也关不掉。
...
result = client.review(
    "2025年报点评.pdf",                     # 路径，或者文档字节
    evidence=[EvidenceItem(field="revenue", value=85.6, unit="亿元",
                           period="2025A", source="2025年年度报告",
                           source_ref="ann:2025A#p12")],
)

print(result.grade.letter)        # A / B / C / D
print(result.blocking)            # 读这一个值就够决定要不要拦
print(result.coverage_note)       # 抽了多少条断言、核了多少
```
...
client.review(document, *, filename="", content_type="",
              evidence=(), evidence_ids=(), timeout=300.0) -> ReviewResult
```
...
| `evidence` | 结构化证据，见 [第 2 节](#2-怎么给证据) |
...
证据是数值能不能被核验的**唯一**来源：一个数字在证据里找不到对应值，它就是无源的。
...
不给证据必然是 D：每个数字都无源，P0 `evidence_required` 逐个开火。这是规则的直接
结果，不是缺陷。
...
这两份都是原文件之外的**新产物**：你交给它的那份文档一个字节都没被动过。
