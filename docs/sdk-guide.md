...
## 5b. 把 Agent 的执行轨迹一起交上去

```python
result = client.review("report.pdf", workflow_audit={"nodes": [...]})
```

给了它，报告里的**工作流审计**那一节才有内容 —— 它查的是**过程**：哪一步没经过
验证器、哪一步声称已验证却有数据回放失败、哪些产出说不清来源。

不给不是「通过」：那节会明写「本次未提供 Agent 执行轨迹，这一块没有审计」。

轨迹动辄几十 KB，塞不进 query，所以给了它就自动改走 JSON 信封（文档走 base64）。
不给时请求体一个字节不变。
...
| `download_review_card_pdf()` → PDF | 要存档的人 | 同一页纸，内容固定、能签字、进合规留痕 |
| `download_review_report()` → Markdown | 要逐条排查的工程师 | 进 diff、进工单、进代码评审 |
| `download_annotated()` → PDF | 要看原文哪里有问题的人 | 高亮画在**原报告**上 |

两份 PDF 别搞混：`download_annotated()` 是**你的报告**加上高亮批注，
`download_review_card_pdf()` 是**审计结论**本身。前者证明「哪一句有问题」，
后者回答「这份能不能发」。
