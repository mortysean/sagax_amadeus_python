"""Sagax Amadeus SDK — Sagax Audit 公网 API 的 Python 客户端。
    ...
    client = SagaxAuditClient(base_url="https://audit.example.com",
                              api_key="sagax_sk_…")
    result = client.review("2025年报点评.md", evidence=[
        EvidenceItem(field="revenue", value=85.6, unit="亿元",
                     source_ref="https://…/annual-report.pdf")])
...
不评级、不生成报告、不存私有知识库。这些全部在 Sagax Audit Cloud 上执行，
SDK 负责把文档发过去、把标注取回来。
