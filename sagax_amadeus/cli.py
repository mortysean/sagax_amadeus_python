...
def cmd_review(args: argparse.Namespace) -> int:
    ...
    with _client(args) as client:
        ...
        if args.card_pdf:
            client.download_review_card_pdf(result.review_id, args.card_pdf)
        _out({"review_id": result.review_id,
              ...
              "card_pdf_saved_to": args.card_pdf})
    ...
    with _client(args) as client:
        if args.action == "list":
            ...
            _out({"saved": client.download_review_card(
                _need(args.review_id, "--review-id"), dest)})
        elif args.action == "card-pdf":
            dest = args.out or f"{args.review_id}-report.pdf"
            _out({"saved": client.download_review_card_pdf(
                _need(args.review_id, "--review-id"), dest)})
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--card-pdf", default=None,
                   help="一页报告的 PDF 版存到这个路径，供存档转发")
    p.set_defaults(func=cmd_review)

    p = sub.add_parser("reviews", help="历史文档审计")
    p.add_argument("action", choices=["list", "get", "annotated", "report",
                                      "card", "card-pdf"])
