"""Sagax Amadeus SDK — 同步客户端。
    ...
    client.download_review_card(result.review_id, "review.html")   # 一页纸，可交互
    client.download_review_report(result.review_id, "review.md")   # 逐条列全
...
class SagaxAuditClient:
    ...
    def review(self, document: Union[str, bytes], *, filename: str = "",
               ...
               workflow_audit: Optional[dict[str, Any]] = None,
        ...
        **证据可以不给。** 服务端会为报告里点了名的指标自己去数据源取真值
        ...
        Args:
            ...
            auto_evidence: 是否允许服务端自动取证。设 False 退回「只用你给的
                证据」，此时不给证据就是每个数字都无源。
            workflow_audit: Agent 的执行轨迹 ``{"nodes": [...]}``。给了它，
                报告里的「工作流审计」那一节才有内容 —— 它查的是**过程**：
                哪一步没经过验证器、哪一步声称已验证却有数据回放失败、哪些
                产出说不清来源。不给则那节明写「未提供轨迹，这一块没有审计」，
                **不会**当成通过。

                轨迹动辄几十 KB，塞不进 query（超 8KB 是 414 而不是一句能看懂
                的报错），所以给了它就改走 JSON 信封发送，文档走 base64。

                **每个节点的字段**（``{"nodes": [ … ]}``）::

                    label            str   步骤名，报告里「问题步骤」那格
                    type             str   调用的接口名
                    status           str   VERIFIED / OK / DONE  已完成
                                           UNVERIFIED / PENDING / SKIPPED
                                           BLOCKED / FAILED      出错
                    inputs           list  [{"label": "上游步骤名"}]
                    output.records   list  [{"value":…, "unit":…, "date":…,
                                             "raw_ref": "<tool_call_id>",
                                             "replay_ok": true}]
                    output.records_total int   必须等于 len(records)
                    output.truncated int   被省略的条数，没截断填 0
                    evidence_refs    list  证据 id 或响应哈希
                    verification     dict  {"status":…, "loss":…, "errors":[]}
                    kind             str   "action"（默认）/ "object"
                    verifies         list  这一步负责验证哪些节点
                    error            str   失败原因

                只有 ``label`` 是必需的，其余缺省时 ``status`` 按 ``PENDING``
                （「没跑到」）处理 —— 不报错，但也**不算通过**。

                四个会被判成问题的组合，都是有意的：

                1. 有 records 却既无 ``type`` 又无 ``inputs`` → ``orphan``，
                   这些数据说不清来源；``kind="object"`` 豁免（产出物的来源是
                   产出它的那个 action）。
                2. ``records_total`` 与 ``len(records)`` 对不上 → 申报数和明细
                   是两套东西。
                3. ``status`` 是 VERIFIED/OK/DONE，却有记录 ``replay_ok=false``
                   → 「已验证」和「回放失败」互斥。
                4. ``status="UNVERIFIED"`` → 这一步没过验证器。这不是错误，
                   是如实标注；它的产出只能当「未验证信息」用。

                ``replay_ok`` 请如实填。没做过回放比对就不要填 true —— 报一个
                自己没做过的检查，比不报更糟。
        ...
        body, body_type = data, ctype
        if workflow_audit:
            # 信封是**自描述**的：服务端认的是 document_base64 这个键，不看
            # Content-Type（那一层拿不到请求头）。query 里的 content_type 说的
            # 仍然是**文档**的 MIME —— 别把它改成 application/json，否则服务端
            # 会拿 JSON 当文档去解析。
            import base64
            import json as _json
            body = _json.dumps(
                {"document_base64": base64.b64encode(data).decode("ascii"),
                 "workflow_audit": workflow_audit}).encode("utf-8")
            body_type = "application/json"
        # idempotent=False 是重点：重发一次 = 多审一遍 + 多扣一次配额。
        return ReviewResult.from_dict(self._with_timeout(timeout).post(
            "/v1/reviews", raw_body=body, content_type=body_type,
    ...
    def download_review_card(self, review_id: str,
                             dest: Optional[str] = None) -> Union[bytes, str]:
        """下载一页报告 · **可交互版**（单文件 HTML）。``dest`` 为 None 时返回字节。

        和 :meth:`download_review_report` 是同一次审阅的两种读法：Markdown 逐条
        列全，给要排查的工程师；这一份是一页纸，给做决定的人 —— 结论、评分、
        站不住的数字、整条执行链，每一步都能展开看调了什么接口、返回了什么。

        单文件、不引 CDN，存下来双击就能看，断网和转发都不掉样式。

        Raises:
            ConflictError: 这次审阅没有一页报告 —— 它早于该功能上线，或渲染
                失败。结论仍在 :meth:`get_review` 与 ``/report`` 里，没有丢。
        """
        return self._t.download(f"/v1/reviews/{_seg(review_id)}/card", dest)

    def download_review_card_pdf(self, review_id: str,
                                 dest: Optional[str] = None
                                 ) -> Union[bytes, str]:
        """下载一页报告 · **静态版**（PDF）。``dest`` 为 None 时返回字节。

        和 :meth:`download_review_card` 是同一份内容的两种形态：PDF 内容固定、
        能签字、能进合规留痕；要展开看每一步调了什么接口、返回了什么，用 HTML。

        Raises:
            ConflictError: 这次审阅没有 PDF 版。它可能单独缺失而 HTML 还在——
                PDF 走 reportlab，没装的部署渲不出来。别拿它的有无推断 HTML。
        """
        return self._t.download(f"/v1/reviews/{_seg(review_id)}/card.pdf", dest)
