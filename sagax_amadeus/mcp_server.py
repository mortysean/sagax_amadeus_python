...
class SagaxMCPServer:
    ...
    def _register_tools(self) -> None:
        ...
        review_schema = {
            ...
            "properties": {
                ...
                "evidence": {
                    ...
                    "description": "结构化证据。**没有证据，文档里的每个数字都"
                                   "算无源** —— 那是 P0 规则的直接结果，不是缺陷"},
        ...
        self._tool("sagax.review",
                   "审一份文档：标出没有依据的数字，逐条给出修改意见，"
                   "并给整份文档一个评级和风险总结。判定由确定性规则做，"
                   "不是模型打分。",
                   review_schema,
    ...
    def _review(self, *, document_path: str,
                ...
                annotated_path: str = "", timeout: float = 300.0) -> Any:
        ...
        result = self.client.review(
            ...
            evidence_ids=evidence_ids or [], timeout=timeout)
        saved = None
        if annotated_path:
