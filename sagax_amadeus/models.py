...
class ServiceVersion(_Model):
    ...
    #: 本部署能交付哪些审阅产物（``annotated`` / ``report`` / ``card`` /
    #: ``card.pdf`` / ``bundle``）。**下载前先查这里，别靠版本号推断** ——
    #: 引擎走镜像、SDK 走 PyPI，两者必然错开；而 1.0.0 时期加过端点却没动
    #: 版本号，所以版本号推不出能力。老引擎不回这个字段，空列表即「不知道」，
    #: 那种情况下按老办法直接试即可。
    review_artifacts: list[str] = dc_field(default_factory=list)
