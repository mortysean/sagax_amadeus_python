...
class HttpTransport:
    """一个到 Sagax Amadeus 云端引擎的 HTTP 连接配置。"""
    ...
    def _once(self, method: str, url: str, body: Optional[bytes],
        ...
        try:
            ...
            raise APIConnectionError(
                f"连不上 Sagax Amadeus 云端引擎: {reason}") from exc
        except OSError as exc:
            raise APIConnectionError(f"连不上 Sagax Amadeus 云端引擎: {exc}") from exc
