...
class RequestShapeTest(MockServerMixin, unittest.TestCase):
    ...
    def test_review_wraps_the_document_in_an_envelope_only_for_traces(self):
        """给了 workflow_audit 才改走 JSON 信封；不给时 body 一个字节不变。

        两件事必须同时成立，少一件都会静默出错：信封里 query 的 content_type
        仍然说的是**文档**的 MIME（改成 application/json 的话服务端会拿信封
        当文档去解析），而没有轨迹时必须走原来的 raw body —— 已经接好的调用
        方升上来不该因为多了个可选参数就换一条编码路径。
        """
        client, server = self.start_mock([(200, {"review_id": "rv_1"}, {})] * 2)
        client.review(b"# doc", filename="r.md")
        plain = server.requests[0]
        self.assertEqual(plain["body"], b"# doc")

        client.review(b"# doc", filename="r.md",
                      workflow_audit={"nodes": [{"id": "n1"}]})
        enveloped = server.requests[1]
        payload = json.loads(enveloped["body"].decode("utf-8"))
        self.assertEqual(base64.b64decode(payload["document_base64"]), b"# doc")
        self.assertEqual(payload["workflow_audit"], {"nodes": [{"id": "n1"}]})
        # query 里的 content_type 描述的是文档，不是请求体
        self.assertIn("content_type=text%2Fmarkdown", enveloped["path"])
